...
const uint32_t Bootloader::an_invalid_signature CORE_FLASH_ALIGNED
...
Bootloader::alloc()
{
	CORE_ASSERT(pubp != NULL);
	CORE_ASSERT(slave_msgp == NULL);
...
Bootloader::fetch()
{
	CORE_ASSERT(subp != NULL);
	CORE_ASSERT(master_msgp == NULL);
	...
	switch (record.type) {
	  case IhexRecord::DATA:
	  {
		  ...
		  if ((endxp <= addresses.pgmadr + setup.pgmlen)
			  ...
			  // ".data" segment
			  CORE_ASSERT(addresses.datapgmadr != NULL);
			  return write(basep + record.offset, record.data, record.count);
		  }
...
Bootloader::remove_all()
{
	...
	bool success;
	(void)success;
	CORE_ASSERT(Flasher::is_aligned(Flasher::get_program_start()));
...
const Bootloader::FlashAppInfo*
Bootloader::get_free_app()
{
	Iterator i = begin();
...
Thread::Return
Bootloader::app_threadf_wrapper(
